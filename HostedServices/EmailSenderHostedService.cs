...
public class EmailSenderHostedService(IDatabaseAccessor db, IOptionsMonitor<SmtpOptions> optionsMonitor,
    ILogger<EmailSenderHostedService> logger) : IEmailSender, IHostedService, IDisposable
{
    ...
    public async Task SendEmailAsync(string email, string subject, string htmlMessage)
    {
        ...
        var affectedRows = await db.CommandAsync($@"INSERT INTO EmailMessages (Id, Recipient, Subject, Message,
            SenderCount, Status) VALUES ({message.MessageId}, {email}, {subject}, {htmlMessage}, 0, {nameof(MailStatus.InProgress)})");
    ...
    public async Task StartAsync(CancellationToken token)
    {
        ...
        FormattableString query = $@"SELECT Id, Recipient, Subject, Message FROM EmailMessages WHERE Status NOT IN
            ({nameof(MailStatus.Sent)}, {nameof(MailStatus.Deleted)})";
